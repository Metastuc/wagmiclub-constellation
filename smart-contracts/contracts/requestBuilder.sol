 ...
 * @notice This is contract encodes HTTP requests to be sent using Chainlink
 * @dev This contract uses hardcoded values for Polygon mumbai
 */

contract requestBuilder is FunctionsClient, ConfirmedOwner {
    ...
    function getRequest(string[2] calldata args) external returns(bytes memory) {
        ...
        req.initializeRequestForInlineJavaScript(source); // Initialize the request with JS code
        if (args.length > 0) req.setArgs(args); // Set the arguments for the request
        bytes memory request = req.encodeCBOR();
